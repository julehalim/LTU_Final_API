...
audio_files_with_majority_label_count = 0
...
for audio_identifier, emotion_probs in audio_file_softmax_sum.items():
    ...
    if ground_truth_entry and ground_truth_entry['need_prediction'] == 'yes':
        # Check for majority label in the ground truth
        emotion_counts = defaultdict(int)
        for emotion in ground_truth_entry['emotion']:
            emotion_counts[emotion] += 1
        majority_emotion = max(emotion_counts, key=emotion_counts.get)
        majority_count = emotion_counts[majority_emotion]
        total_count = len(ground_truth_entry['emotion'])
        
        if majority_count > total_count / 2:
            ground_truth_labels.append(majority_emotion)
            predicted_labels.append(most_probable_emotion)
            print(f"Audio ID: {audio_identifier}")
            print(f"Ground truth majority emotion: {majority_emotion}")
            ...
            audio_files_with_majority_label_count += 1
...
# Output the total count of audio files with a majority ground truth label and needing prediction
print(f"Total number of audio files with a majority ground truth label and needing prediction: {audio_files_with_majority_label_count}")
...
if ground_truth_labels and predicted_labels:
    ...
    precision_unweighted = precision_score(ground_truth_labels, predicted_labels, average='macro', zero_division=0)
    recall_unweighted = recall_score(ground_truth_labels, predicted_labels, average='macro', zero_division=0)
    f1_unweighted = f1_score(ground_truth_labels, predicted_labels, average='macro', zero_division=0)
    
    # Calculate precision, recall, and F1 score (weighted)
    precision_weighted = precision_score(ground_truth_labels, predicted_labels, average='weighted', zero_division=0)
    recall_weighted = recall_score(ground_truth_labels, predicted_labels, average='weighted', zero_division=0)
    f1_weighted = f1_score(ground_truth_labels, predicted_labels, average='weighted', zero_division=0)
