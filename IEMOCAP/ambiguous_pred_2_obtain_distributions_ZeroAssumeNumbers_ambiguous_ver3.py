...
# Initialize dictionaries to store softmax probabilities after each token
...
for entry in json_data:
    ...
    for step in steps:
        ...
        # Apply softmax to the logits of each token immediately and store the probabilities
        if current_emotion:
            emotion_logits = [
            ...
            # Apply softmax to the token logits
            softmax_probs = softmax(emotion_logits)

            # Store the softmax probabilities for each emotion
        ...
        # Handle percentage or other related tokens (reset current emotion)
